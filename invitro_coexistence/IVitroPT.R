setwd("~/Documents/University/Manuscripts/Brochet_strain-interactions/Datasets/Invitro_pairwise_transfer")
# Packages --------
...
# This table was already modified to re-assign certain samples that were not correctly assigned due to mis-labelling during libraries preparation

data <- read.table("invitro_transferpairs.txt", h=T, check.names = FALSE) #table with raw counts results
#P2_57_1 excluded - no bacteria - and consequently also P3_57_1
...
# to quantify contaminations (check contaminations quantitatively)
...
prop3$variable <- sub("[P]3_", "", prop3$variable)
...
# set a threshold to exclude contaminated samples. For now I will not
# yet exclude samples.
...
data1 <- read.table("invitro_transferpairs.txt", h=T, check.names = FALSE) #table with raw counts results
...
data1$variable <- sub("[P]3_", "", data1$variable)
...
for (variableName in levels(norm_pairs$variable)) {
  ...
  p1 = ggplot(variableData, aes(transfer, Normalised_counts)) + 
    ...
    scale_y_log10() +
    coord_cartesian(ylim=c(1e0,1e+10)) +
    #ggtitle(variableName) +
    xlab("Transfer") + 
    ylab("Normalised counts") +
    scale_color_manual(values = colors) +
